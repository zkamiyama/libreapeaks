...
-- or RPKX_ForceBuild. RPKX_Status is observation, not a build trigger.
...
local function quit(err)
  ...
  -- Saving avoids the normal unsaved-project prompt; errors must not leave a
  -- ReaScript dialog hiding the actual CI failure until the external timeout.
  local ok,save_error=pcall(reaper.Main_SaveProjectEx,0,root..'/saved.rpp',0)
  if not ok then log('save_error',save_error) end
  log('finished',true);f:close();closed=true
...
local function main()
  ...
  local actions={}
  -- REAPER 7.79's Lua wrapper accepts a numeric section ID. The native C API
  -- uses KbdSectionInfo*. Do not pass a C-style pointer to the Lua wrapper.
  for i=0,65535 do
    local id,name=reaper.kbd_enumerateActions(0,i)
    if not id or id<=0 then break end
    if type(name)=='string' then actions[name]=id end
  end
  local af=assert(io.open(root..'/actions.txt','w'))
  for name,id in pairs(actions) do
    if name:find('peak') or name:find('Peak') or name:find('offline') or name:find('online') or name:lower():find('reverse') then af:write(id,'\t',name,'\n') end
  end
  af:close()
  local function action(name)
    local id=actions[name];if not id then error('Missing host action: '..name) end
    log('action',name);log('action_id',id);reaper.Main_OnCommand(id,0)
    log('action_returned',name)
  end
  ...
  tick=function()
    local ok,err=xpcall(function()
      local src=source()
      if not src then
        if reaper.time_precise()-initial>5 then error('No imported source') end
      ...
      if not acted and age>1.0 and (not plugin or st==2 or st==0 or st==-1 or st==-2) then
        ...
        -- Source identity can change with actions; observe again next turn.
        reaper.defer(tick);return
      end
