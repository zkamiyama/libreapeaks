...
repo = Path.cwd()
root = Path(sys.argv[1] if len(sys.argv) > 1 else "/tmp/f32-finite-whole")
...
try:
    ...
    for index, case in enumerate(cases):
        ...
        for old in media.rglob(peak_name):
            old.unlink()
        case_dir = Path(tempfile.mkdtemp(prefix=f"f32-finite-whole-{index:03d}-"))
        ...
        if (index + 1) % 16 == 0 or index + 1 == len(cases):
            print(f"REAPER_F32_WHOLE_PROGRESS={index + 1}/{len(cases)}", flush=True)
finally:
    xvfb.terminate()
    try:
        xvfb.wait(timeout=3)
...
failures = 0
interesting = (
    "F32_SPECTROGRAM_EXACT_STATS",
    "f32 spectrogram mismatch",
    "layer header table differs",
    "non-spectrogram layer",
    "RPKL header differs",
    "whole RPKL file differs",
    "assertion",
)
for index, case in enumerate(cases):
    name = case["name"]
    ...
    completed = subprocess.run(
        [
            ...
            "--test",
            "reaper_spectrogram_f32_exact",
            "reaper779_f32_rpkl_spectrogram_is_byte_identical",
            "--",
            "--ignored",
            "--exact",
            "--nocapture",
    ...
    (results / f"{name}.whole-test.txt").write_text(completed.stdout, encoding="utf-8")
    if completed.returncode == 0:
        detail = next(
            (line for line in completed.stdout.splitlines() if "F32_SPECTROGRAM_BYTE_IDENTICAL" in line),
            "",
        )
        ...
        summary.append(f"FAIL {name} rc={completed.returncode} " + " | ".join(lines[-16:]))
        print(summary[-1], flush=True)
    if (index + 1) % 16 == 0 or index + 1 == len(cases):
        print(
            f"F32_WHOLE_STRESS_PROGRESS={index + 1}/{len(cases)} failures={failures}",
            flush=True,
        )

summary.append(f"F32_WHOLE_STRESS_TOTAL={len(cases)} WHOLE_FILE_FAILURES={failures}")
