...
repo = Path.cwd()
root = Path(sys.argv[1] if len(sys.argv) > 1 else "/tmp/spectrogram-f32")
...
try:
    ...
    for index, case in enumerate(cases):
        ...
        for old in media.rglob(peak_name):
            old.unlink()
        case_dir = Path(tempfile.mkdtemp(prefix=f"spectrogram-f32-{index:03d}-"))
        ...
        print(f"REAPER_F32_PROGRESS={index + 1}/{len(cases)} {name}", flush=True)
finally:
    xvfb.terminate()
    try:
        xvfb.wait(timeout=3)
...
for index, case in enumerate(cases):
    ...
    print(f"F32_ORACLE_PROGRESS={index + 1}/{len(cases)} failures={failures}", flush=True)

summary.append(f"F32_SPECTROGRAM_TOTAL={len(cases)} WHOLE_FILE_FAILURES={failures}")
