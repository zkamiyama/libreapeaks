...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let source = PathBuf::from(args.next().ok_or("missing source WAV path")?);
    ...
    let packed = generate_pcm24_reaper(&raw, &options, mode)?;
    let i32_cache = generate_pcm24_i32_reaper(&pcm_i32, &options, mode)?;
