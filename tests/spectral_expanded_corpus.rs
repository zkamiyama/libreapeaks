    ...
    match spec[0] {
        "tone" => {
            ...
            for i in 0..frames {
                for c in 0..channels {
                    ...
                    out.push(q16(amp * (2.0 * PI * f * i as f64 / sr as f64 + phase).sin()));
                }
            }
            out
        }
        ...
        "pattern" => {
            ...
            for i in 0..frames {
                for c in 0..channels {
                    let v = match mode {
                        "alt" => if (i + c) & 1 == 1 { 32767 } else { -32768 },
                        "saw" => ((((i * 3 + c * 7) % 31) as f64 / 30.0 * 2.0 - 1.0) * 24000.0) as i32,
...
fn reaper779_expanded_fresh_process_spectral_corpus_is_exact() {
    ...
    for line in include_str!("data/spectral_expanded_oracle.tsv").lines() {
        ...
        let got = if sample_type == "i16" {
            build_fine_spectral(&pcm_i16(sr, channels, frames, &spec), frames, channels, sr, division).unwrap()
        } else {
            assert_eq!(sample_type, "f32");
            build_fine_spectral_f32(&pcm_f32(channels, frames, &spec), frames, channels, sr, division).unwrap()
        };
    ...
    eprintln!("SPECTRAL_EXPANDED exact_cases={cases} exact_codes={codes}");
}
